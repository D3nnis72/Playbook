...
**Downstream of the spec:** the spec is the contract between the canvas and this plan. Target is what to build. Scope is where it lives and what already exists. This plan adds source-file locations and step-level detail. Carry those spec names through; do not rename them into a more general category. The spec's Verification walkthrough becomes real checks (prefer existing suites); do not invent a unit test per Approach unit unless an outcome requires it. Map Target outcomes onto Scope homes. If Approach is present, every unit named there gets a file and a task under the name the spec gave it. If Approach (or its `### Shapes`) lists what a unit carries, use those fields — do not silently invent a different model. If a named unit has no home in the plan, either the plan is incomplete or the spec's design changed — resolve it before execution rather than silently renaming.
