...
const PLAYBOOK_VERSION = readPlaybookVersion();
const TELEMETRY_DISABLE_ENV_VARS = [
  'PLAYBOOK_DISABLE_TELEMETRY',
  'DISABLE_TELEMETRY',
  'CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC'
];
const PLAYBOOK_TELEMETRY_DISABLED = TELEMETRY_DISABLE_ENV_VARS.some(name => isTruthyEnv(process.env[name]));
...
function brandMarkup() {
  const version = escapeHtmlText(PLAYBOOK_VERSION);
  const text = 'Playbook v' + version;
